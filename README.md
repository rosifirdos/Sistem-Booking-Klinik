...
Berikut adalah representasi Class Diagram yang menggambarkan struktur, kelas, dan hubungan dalam proyek ini:
classDiagram
    direction LR
    ...
    class Application {
        ...
        +populate_doctor_cards()
        +populate_booking_table()
        +populate_doctor_comboboxes()
        +add_new_booking(doctorId, patientName, ...)
        +delete_booking(bookingId)
        +show_doctors_view()
        +show_bookings_view()
        +show_chatbot_view()
        +send_message_to_chatbot(message)
        +display_chatbot_response(response, history)
        +display_chatbot_error(error)
    }
    ...
    class BookingService {
        -db_manager: DatabaseManager
        +insert_initial_data()
        +get_all_doctors_with_specialty(): list
        +get_all_specialties(): list
        +get_doctors_by_specialty(specialty_name: str): list
        +get_doctor_schedules(doctor_id, date, include_booked): list
        +add_booking(schedule_id, doctor_id, ...)
        +get_all_bookings(): list
        +delete_booking(booking_id: int): (bool, str)
    }

    class BookingDialog {
        -parent_window: Application
        -doctor_id: int
        -doctor_name: str
        -specialty: str
        -initial_date: QDate
        +patientNameInput: QLineEdit
        +patientPhoneInput: QLineEdit
        +bookingDateInput: QDateEdit
        +scheduleIdComboBox: QComboBox
        +booking_confirmed: pyqtSignal
        +init_ui()
        +populate_schedule_combobox()
        +confirm_booking()
    }
    ...
    class GeminiChatbotService {
        -model
        -chat_history
        +initialize_model(api_key: str): bool
        +send_message(prompt: str, history: list): (str, list)
    }

    class GeminiChatbotWorker {
        -api_key: str
        -user_prompt: str
        -chat_history: list
    ...
    Application "1" -- "1" BookingService : uses >
    Application "1" -- "1" GeminiChatbotService : uses >
    Application "1" -- "1" BookingDialog : creates/uses >
    Application "1" -- "1" GeminiChatbotWorker : creates/manages >

    BookingService "1" -- "1" DatabaseManager : uses >
    BookingDialog "1" -- "1" BookingService : uses >
    GeminiChatbotWorker "1" -- "1" GeminiChatbotService : uses >
    ...
    Doctors "1" -- "*" Schedules : has >
    Doctors "1" -- "*" Bookings : has >
    Schedules "1" -- "1" Bookings : associated with >

    note "DatabaseManager dan BookingService mengelola Doctors, Schedules, dan Bookings" as ModelManagementNote
    DatabaseManager .. ModelManagementNote
    BookingService .. ModelManagementNote
